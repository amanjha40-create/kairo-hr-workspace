...
import type { VerificationStatus } from "@/lib/workspace-types";
...
function Sparkline({ data, positive }: { data: number[]; positive: boolean }) {
  ...
  const max = Math.max(...data),
    min = Math.min(...data);
  const points = data
    .map((v, i) => `${(i / (data.length - 1)) * w},${h - ((v - min) / Math.max(1, max - min)) * h}`)
    .join(" ");
  return (
    <svg width={w} height={h} className="opacity-70">
      <polyline
        points={points}
        fill="none"
        stroke={positive ? "hsl(160 60% 40%)" : "hsl(0 60% 55%)"}
        strokeWidth="1.5"
        strokeLinecap="round"
...
export function SectionCard({
  ...
  return (
    <Card
      className={cn(
        "p-0 rounded-2xl border-border/60 overflow-hidden shadow-[var(--shadow-soft)]",
        className,
      )}
    >
...
export function EmptyState({
  ...
  return (
    <div className="py-16 px-6 text-center flex flex-col items-center">
      ...
      {action && (
        <Button className="btn-premium rounded-xl mt-5" onClick={action.onClick}>
          {action.label}
        </Button>
      )}
    </div>
...
export function StatCard({
  ...
  return (
    ...
    >
      ...
      <div className={cn("mt-2 text-2xl font-semibold tabular-nums tracking-tight", toneStyle)}>
        {value}
      </div>
    </Comp>
  );

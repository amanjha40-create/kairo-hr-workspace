...
import {
  Plus, MailPlus, ShieldCheck, Hourglass, Loader2, CheckCircle2,
  ArrowRight, UserPlus, FileText, Share2, MessageCircleWarning, ShieldOff,
  AlertTriangle, MailOpen, MailCheck, Sparkles, EyeOff,
} from "lucide-react";
...
function Overview() {
  const { people, requests, inboundRequests, attention, setInviteOpen, emptyMode, setEmptyMode } = useDashboard();
  const { empty } = Route.useSearch();
  const isEmpty = emptyMode || people.length === 0;

  const activeInvitations = people.filter((p) => p.invitationStatus === "Sent" || p.invitationStatus === "Opened").length;
  const awaitingCandidate = people.filter((p) =>
    p.workspaceVerificationStatus === "Waiting for Candidate" ||
    p.invitationStatus === "Sent" || p.invitationStatus === "Opened",
  ).length;
  const inVerification = people.filter((p) =>
  ...
  const inboundOpen = inboundRequests.filter((r) => r.status === "New" || r.status === "In Review" || r.status === "Clarification Requested").length;

  const kpis = [
    { key: "invites", label: "Active Trust Invitations", value: activeInvitations, icon: MailPlus, desc: "Awaiting candidate acceptance", to: "/app/invitations", search: undefined },
    { key: "inbound", label: "Employment Verifications", value: inboundOpen, icon: ShieldCheck, desc: "Incoming from other organizations", to: "/app/verifications", search: undefined },
    { key: "awaiting", label: "Awaiting Candidate", value: awaitingCandidate, icon: Hourglass, desc: "Needs candidate action", to: "/app/invitations", search: undefined },
    { key: "in-verif", label: "In Verification", value: inVerification, icon: Loader2, desc: "Currently being processed", to: "/app/invitations", search: undefined },
    { key: "completed", label: "Completed This Month", value: completedThisMonth, icon: CheckCircle2, desc: "Finalized this month", to: "/app/invitations", search: undefined },
  ] as const;

  const recentInbound = inboundRequests.slice(0, 6);
  void empty;
    ...
    <div>
      <PageHeader
        ...
        actions={
          <>
            {emptyMode ? (
              <Button variant="outline" onClick={() => setEmptyMode(false)} className="rounded-xl" size="sm">
                Exit empty preview
              </Button>
            ) : import.meta.env.DEV ? (
              <Button variant="outline" onClick={() => setEmptyMode(true)} className="rounded-xl" size="sm" title="Development only">
      ...
      {isEmpty ? (
        <SectionCard title="Get started" description="Your workspace is ready.">
          <div className="py-16 px-6 flex flex-col items-center text-center">
            ...
            <h2 className="text-xl sm:text-2xl font-semibold tracking-tight">Invite your first candidate</h2>
            <p className="text-sm text-muted-foreground mt-2 max-w-md">
              Begin by inviting a candidate to securely share their professional information
              through a Trust Invitation.
                    ...
                    <div className="flex-1 min-w-[220px]">
                      ...
                      <div className="text-[11px] text-muted-foreground mt-1 flex items-center gap-2 flex-wrap">
                        <span className="inline-flex items-center gap-1">
                          <span className="h-1.5 w-1.5 rounded-full bg-current opacity-60" /> {a.status}
                        </span>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/60">
                        ...
                        <th className="text-left font-medium py-2.5 px-3 hidden md:table-cell">Requesting org</th>
                        <th className="text-left font-medium py-2.5 px-3">Status</th>
                        <th className="text-left font-medium py-2.5 px-3 hidden lg:table-cell">Received</th>
                        <th className="text-right font-medium py-2.5 px-5">Open</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/60">
                      {recentInbound.map((r) => (
                        <tr key={r.id} className="hover:bg-foreground/[0.02] transition-colors group">
                          ...
                          <td className="py-3 px-3 text-sm text-muted-foreground hidden md:table-cell">{r.requestingOrg.name}</td>
...
function AttentionIcon({ reason }: { reason: string }) {
  ...
  if (r.includes("completed")) { Icon = CheckCircle2; cls = "bg-success/15 text-success"; }
  else if (r.includes("could not") || r.includes("unable")) { Icon = ShieldOff; cls = "bg-destructive/10 text-destructive"; }
  else if (r.includes("clarification")) { Icon = MessageCircleWarning; cls = "bg-warning/15 text-warning-foreground"; }
  else if (r.includes("expires")) { Icon = MailOpen; cls = "bg-warning/15 text-warning-foreground"; }
  else if (r.includes("accept")) { Icon = MailPlus; cls = "bg-info/15 text-info-foreground"; }
  else if (r.includes("incomplete")) { Icon = Hourglass; cls = "bg-warning/15 text-warning-foreground"; }
  return <div className={`h-8 w-8 rounded-full flex items-center justify-center ${cls}`}><Icon className="h-4 w-4" /></div>;
}

function RecentActivity() {
  const { people } = useDashboard();
  const items = people
    .flatMap((p) => p.personActivity.slice(0, 2).map((a) => ({ ...a, personName: p.name, personId: p.id })))
    .sort((a, b) => new Date(b.at).getTime() - new Date(a.at).getTime())
    .slice(0, 10);
  if (items.length === 0) {
    return <EmptyState icon={Sparkles} title="No recent activity" description="Activity will appear here as candidates and verifications progress." />;
  }
  return (
      ...
      {items.map((a) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm"><span className="font-medium">{a.personName}</span> <span className="text-muted-foreground">· {a.label.toLowerCase()}</span></div>
            <div className="text-[11px] text-muted-foreground mt-0.5">{formatDistanceToNow(new Date(a.at), { addSuffix: true })} · {a.actor}</div>
          </div>
        </Link>
      ))}
    </div>
...
function ActivityIcon({ kind }: { kind: string }) {
  const map: Record<string, { Icon: typeof CheckCircle2; cls: string }> = {
    ...
    "clarification-req": { Icon: MessageCircleWarning, cls: "bg-warning/15 text-warning-foreground" },
    "clarification-recv": { Icon: MessageCircleWarning, cls: "bg-warning/15 text-warning-foreground" },
  ...
  return <div className={`h-7 w-7 rounded-full flex items-center justify-center shrink-0 ${cls}`}><Icon className="h-3.5 w-3.5" /></div>;
}

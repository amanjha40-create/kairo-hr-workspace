...
const routePermissionRules: Array<{
...
}> = [
  {
    ...
    message: "You don't have permission to view People in this workspace.",
  },
  {
    prefix: "/app/invitations",
    action: "modify_invitation",
    message: "You don't have permission to view Trust Invitations in this workspace.",
  },
  {
    prefix: "/app/verifications",
    action: "modify_verification",
    message: "You don't have permission to view Employment Verifications in this workspace.",
  },
  {
    prefix: "/app/team",
    action: "manage_team",
    message: "You don't have permission to view Team in this workspace.",
  },
  {
    prefix: "/app/settings",
    action: "save_settings",
    message: "You don't have permission to view Settings in this workspace.",
  },
];

function getRoutePermission(path: string) {
  return (
    routePermissionRules.find(
...
function AppLayout() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key.toLowerCase() === "n") {
        if (!can("invite_candidate")) return;
        e.preventDefault();
        setInviteOpen(true);
        return;
      }
  ...
  const fullName =
    (user?.user_metadata?.full_name as string | undefined) ?? user?.email ?? "Workspace User";
    ...
    <div className="min-h-screen flex bg-[hsl(var(--background))]">
      ...
      <aside className="hidden lg:flex w-64 flex-col border-r border-border/60 bg-background sticky top-0 h-screen">
        ...
        <div className="p-3 border-t border-border/60">
          <Link
            to="/app/profile"
            className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-foreground/[0.04]"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-border/60 bg-background/95 backdrop-blur flex items-center px-3 md:px-6 sticky top-0 z-30 gap-2 md:gap-3">
          ...
          <div className="relative flex-1 max-w-xl">
            ...
            />
            <kbd className="hidden sm:inline-flex absolute right-3 top-1/2 -translate-y-1/2 h-5 items-center px-1.5 rounded-md border border-border/60 bg-background text-[10px] text-muted-foreground font-mono">
              /
            </kbd>
          </div>
          ...
          {can("invite_candidate") ? (
            <Button
              ...
              className="btn-premium rounded-xl hidden sm:inline-flex"
              size="sm"
            >
            ...
            <Badge
              variant="outline"
              className="hidden sm:inline-flex h-8 items-center text-[11px] font-medium"
            >
              {role} · read-only
            </Badge>
          )}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-xl">
              ...
              <DropdownMenuItem asChild>
                <Link to="/app/profile">
                  <CircleUser className="h-4 w-4 mr-2" /> My account
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
